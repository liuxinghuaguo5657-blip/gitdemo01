...
## 6. 挑选提交 (Cherry-picking)

- `git cherry-pick <提交哈希值>`: 复制指定提交的更改，并在当前分支上创建一个包含这些更改的新提交。

## 7. 远程交互 (`fetch` vs `pull`)

- `git fetch`: 从远程仓库下载最新的数据（提交、分支、标签），但**不**会自动合并到您当前的工作中。这是一个安全的操作。
- `git pull`: 从远程仓库下载最新数据，并**立即**尝试将远程分支的改动合并到当前分支。相当于 `git fetch` + `git merge`。

## 8. 工作树 (Worktree)

- `git worktree add -b <新分支> <路径>`: 创建一个新分支，并将其检出到一个新的工作目录（工作树）中，以便并行工作。
- `git worktree list`: 列出所有链接到当前仓库的工作树。
- `git worktree remove <路径>`: 移除一个额外的工作树。